...
namespace LittleTetris{
    public partial class Form1 : Form{
        ...
        public void FillField(){
            ...
            gr.DrawRectangle(Pens.Pink, k, k, (width - 1) * k, (height - 1) * k);
            for (int i = 0; i < width; i++)
                for (int j = 0; j < height; j++)
                    gr.FillRectangle(Brushes.IndianRed, i * k, j * k, (k - 1) * field[i, j], (k - 1) * field[i, j]);
            for (int i = 0; i < 4; i++)
                gr.FillRectangle(Brushes.Orange, shape[1, i] * k, shape[0, i] * k, k - 1, k - 1);
